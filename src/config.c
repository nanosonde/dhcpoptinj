...
static int            parseKeyValue(const char *key, const char *value, const char *filePath, unsigned lineNo);
...
struct Config *conf_parseOpts(int argc, char *const *argv)
{
   ...
   struct DHCPOptList *dhcpOptList = dhcpOpt_count(cmdDHCPOptList) ? cmdDHCPOptList : fileDHCPOptList;
...
static void parseOption(struct Config *config, int option, char *arg, enum Source source)
{
   ...
   {
      fprintf(stderr,
              "The keyword \"%s\" doesn't make sense in a configuration "
              "file\n",
              options[option].name);
      exit(EXIT_FAILURE);
   }
...
static void validateOptionCombinations(void)
{
   ...
   if (totalOptionCount(OPT_IGNORE_EXISTING_OPT) && totalOptionCount(OPT_REMOVE_EXISTING_OPT))
   {
      fprintf(stderr,
              "Both %s%s and %s%s cannot be used at the same time\n",
...
static char *trim(char *text)
{
   ...
   for (char *ch = text + strlen(text) - 1; isspace((int)*ch) || *ch == '\'' || *ch == '\"'; *ch-- = '\0')
      ;
...
static int parseKeyValue(const char *key, const char *value, const char *filePath, unsigned lineNo)
{
   for (int option = 0; option < OPT_COUNT; ++option)
   {
      ...
      {
         fprintf(stderr,
                 "Failed to parse \"%s\" at line %u: keyword \"%s\" requires an argument\n",
                 filePath,
                 lineNo,
                 options[option].name);
         exit(EXIT_FAILURE);
      ...
      {
         fprintf(stderr,
                 "Failed to parse \"%s\" at line %u: keyword \"%s\" does not take an argument\n",
                 filePath,
                 lineNo,
                 options[option].name);
         exit(EXIT_FAILURE);
...
static bool validConfOption(int option)
{
   return !(option == OPT_CONF_FILE || option == OPT_HELP || option == OPT_VERSION || option == OPT_LIST_OPTS);
}
